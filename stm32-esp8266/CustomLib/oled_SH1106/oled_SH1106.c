...
#define OLED_ADDRESS 0x3C << 1 // for stm32 7-bit i2c device address should be left shifted by 1.
#define DISP_ON 0xAF
...
static uint8_t char_cnt;
static uint8_t num_cnt;
static uint8_t battery[14] = {0xFF, 0x81, 0x81, 0x81, 0x81, 0x81, 0x81, 0x81, 0x81,
							  0x81, 0x81, 0x81, 0xC3, 0x3C};
static uint8_t signal[9] = {0x80, 0x00, 0xC0, 0x00, 0xF0, 0x00, 0xFC, 0x00, 0xFF};
static uint8_t bar[128];

static uint8_t dispBuffer[8][128];
static uint8_t disp_row;
static uint8_t disp_column;
...
void oled_plot(float current_x, float current_y)
{
...
{
	memcpy((uint8_t *)dispBuffer, binArray, sizeof(dispBuffer));
}
